        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('classes.schedules.update', ['class' => $class, 'schedule' => $schedule]) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="room" name="room" type="text" class="w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-gray-400 focus:border-gray-400 transition" :value="old('room', $schedule->room)" required />
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <x-text-input id="start_time" name="start_time" type="time" value="{{ old('start_time', $schedule->start_time) }}"  class="w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-gray-400 focus:border-gray-400 transition" :value="old('start_time', $schedule->start_time)" required />
                                    <x-input-error class="mt-2" :messages="$errors->get('start_time')" />
                                </div>

                                <div>
                                    <x-input-label for="end_time" value="Horário de Término" />
                                    <x-text-input id="end_time" name="end_time" type="time" value="{{ old('end_time', $schedule->end_time) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-gray-400 focus:border-gray-400 transition" :value="old('end_time', $schedule->end_time)" required />
                        ...
                        <div class="mt-6">
                            ...
                            <div class="mt-2 grid grid-cols-2 md:grid-cols-5 gap-4">
                                <label class="inline-flex items-center bg-white px-4 py-2 rounded-lg border border-gray-300 shadow-sm hover:bg-gray-50 transition cursor-pointer">
                                    <input type="checkbox" name="days[]" value="monday" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ in_array('monday', old('days', $schedule->days->pluck('day_of_week')->toArray())) ? 'checked' : '' }}>
                                    <span class="ml-2">{{ \App\Helpers\DateHelper::translateDayOfWeek('monday') }}</span>
                                </label>
                                <label class="inline-flex items-center bg-white px-4 py-2 rounded-lg border border-gray-300 shadow-sm hover:bg-gray-50 transition cursor-pointer">
                                    <input type="checkbox" name="days[]" value="tuesday" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ in_array('tuesday', old('days', $schedule->days->pluck('day_of_week')->toArray())) ? 'checked' : '' }}>
                                    <span class="ml-2">{{ \App\Helpers\DateHelper::translateDayOfWeek('tuesday') }}</span>
                                </label>
                                <label class="inline-flex items-center bg-white px-4 py-2 rounded-lg border border-gray-300 shadow-sm hover:bg-gray-50 transition cursor-pointer">
                                    <input type="checkbox" name="days[]" value="wednesday" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ in_array('wednesday', old('days', $schedule->days->pluck('day_of_week')->toArray())) ? 'checked' : '' }}>
                                    <span class="ml-2">{{ \App\Helpers\DateHelper::translateDayOfWeek('wednesday') }}</span>
                                </label>
                                <label class="inline-flex items-center bg-white px-4 py-2 rounded-lg border border-gray-300 shadow-sm hover:bg-gray-50 transition cursor-pointer">
                                    <input type="checkbox" name="days[]" value="thursday" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ in_array('thursday', old('days', $schedule->days->pluck('day_of_week')->toArray())) ? 'checked' : '' }}>
                                    <span class="ml-2">{{ \App\Helpers\DateHelper::translateDayOfWeek('thursday') }}</span>
                                </label>
                                <label class="inline-flex items-center bg-white px-4 py-2 rounded-lg border border-gray-300 shadow-sm hover:bg-gray-50 transition cursor-pointer">
                                    <input type="checkbox" name="days[]" value="friday" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ in_array('friday', old('days', $schedule->days->pluck('day_of_week')->toArray())) ? 'checked' : '' }}>
                        ...
                        <div class="mt-6">
                            <label class="inline-flex items-center bg-white px-4 py-2 rounded-lg border border-gray-300 shadow-sm hover:bg-gray-50 transition cursor-pointer">
                                <input type="checkbox" name="is_substitute" id="is_substitute" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" 
                                    {{ $schedule->is_substitution ? 'checked' : '' }}>
                                <span class="ml-2">É substituição?</span>
                            </label>
                        </div>

                        <div id="original_teacher_div" class="{{ $schedule->is_substitution ? '' : 'hidden' }} mt-6">
                            ...
                            <select id="original_teacher_id" name="original_teacher_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-gray-400 focus:border-gray-400 transition">
                                ...
                                @foreach($teachers as $teacher)
                                    <option value="{{ $teacher->id }}" {{ $schedule->original_teacher_id == $teacher->id ? 'selected' : '' }}>
                                        {{ $teacher->name }}
                                    </option>
                                @endforeach
                            </select>
